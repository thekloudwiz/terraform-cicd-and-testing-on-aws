...
resource "aws_sns_topic_subscription" "manual_approval_sns_subscription" {
  ...
  protocol  = "email"
  endpoint  = "yourmail@domain.com"  # Replace with your email address
}
...
module "module-aws-tf-cicd" {
  ...
  connection_timeout  = 30  # or your desired timeout
          ...
          action = [
            {
              ...
              configuration = {
                ...
                FullRepositoryId = "${var.github_owner}/${local.module_aws_tf_cicd_repository_name}"
                BranchName     = "main"
              }
              input_artifacts = []
          ...
          action = [
            {
              ...
              configuration = {
                ConnectionArn    = local.codestar_connection_arn
                FullRepositoryId = "${var.github_owner}/${local.example_production_workload_repository_name}"
                BranchName     = "main"
              }
              input_artifacts = []
              #  Store the output of this stage as 'source_output_artifacts' in connected the Artifacts S3 Bucket
