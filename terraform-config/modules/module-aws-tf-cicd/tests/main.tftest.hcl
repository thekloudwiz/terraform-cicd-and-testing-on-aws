...
variables {
  github_token = {
    # checkov:skip=CKV_SECRET_6: "OAuth token is intentionally stored here for testing purposes"
    oauth_token = "insert-your-oauth-token"
  }
          ...
          action = [
            {
              ...
              version  = "1"
              configuration = {
                ConnectionArn = "arn:aws:codeconnections:<region>:<AWS-ACCOUNT-ID>:connection/<connectionID>"
                FullRepositoryId = "owner/test-module-1"
                BranchName = "main"
              }
              input_artifacts = []
...
run "input_validation" {
  ...
  variables {
    ...
    # GitHub - Intentional repository name that is longer than max of 40 characters
    github_repos = {
      # Test Module 1 Repo
      test_module_1 : {
...
run "e2e_test" {
  ...
  # Assertions
  # GitHub - Ensure repositories have correct names after creation
  assert {
    condition     = github_repository.repos["test_module_1"].name == "test-module-1"
    error_message = "The GitHub Repo name (${github_repository.repos["test_module_1"].name}) didn't match the expected value."
  }
  #   assert {
  #   condition     = aws_codestarconnections_connection.github.provider_type == "GitHub"
  #   error_message = "The CodeStar connection provider type is not GitHub"
  # }
