...
variable "connection_timeout" {
  type        = number
  description = "Timeout in seconds for the connection approval wait"
  default     = 300
}

variable "wait_for_connection" {
  type        = bool
  description = "Whether to wait for the connection to be available"
  default     = true
}
...
variable "github_repos" {
  ...
  description = "Collection of GitHub Repositories to create"
  default     = {}

  validation {
    condition = alltrue([
      for repo in values(var.github_repos) :
      length(repo.repository_name) <= 100
    ])
    error_message = "Repository name cannot exceed 100 characters in length."
  }

  validation {
    condition = alltrue([
      for repo in values(var.github_repos) :
      can(regex("^[a-zA-Z0-9_.-]+$", repo.repository_name))
    ])
    error_message = "Repository names must contain only letters, numbers, underscores, dots, and hyphens."
  }
