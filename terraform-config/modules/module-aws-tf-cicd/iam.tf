...
# Create policy attachment for the eventbridge role
resource "aws_iam_role_policy_attachment" "eventbridge_invoke_tf_workshop_event_bus" {
  count = var.create_cloudwatch_service_role ? 1 : 0

  role       = aws_iam_role.eventbridge_invoke_tf_workshop_event_bus[0].id
...
# Create policy attachment for the code eventbridge_invoke_pipeline role
